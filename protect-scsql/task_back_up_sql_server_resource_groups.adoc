...
+
검색 상자에 자원 그룹 이름을 입력하거나 * * 를 선택한 다음 태그를 선택하여 자원 그룹을 검색할 수image:../media/filter_icon.gif["필터 아이콘"]있습니다. 그런 다음 * * 를 선택하여 필터 창을 닫을 수image:../media/filter_icon.gif["필터 아이콘"]있습니다.
