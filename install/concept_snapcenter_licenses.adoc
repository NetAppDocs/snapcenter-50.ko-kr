...
하나 이상의 SnapCenter 라이센스를 설치해야 합니다. 라이센스를 추가하는 방법에 대한 자세한 내용은 또는 를 link:../install/concept_snapcenter_standard_controller_based_licenses.html["SnapCenter 표준 컨트롤러 기반 라이센스를 추가합니다"] link:../install/concept_snapcenter_standard_capacity_based_licenses.html["SnapCenter 표준 용량 기반 라이센스 추가"]참조하십시오.
